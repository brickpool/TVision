...
BEGIN {
  ...
  sub import {
    my $caller = caller();
    $_ = fields::LOP->init($caller)
      ->create_constructor()
      ->warnings_strict()
      ->have_accessors('slot');

    fields::LOP->init( __PACKAGE__ )
      ->import_methods($caller, qw( extends ));
  }

  sub extends {
    my $caller = caller();
    fields::LOP->init( $caller )
      ->extend_class( @_ );
  }
...

{
  ...
  is_deeply(
    fields::LOP->init('Point')->get_attributes(),
    { x => 1, y => 2 },
...

{
  ...
  is_deeply(
    fields::LOP->init('Point3D')->get_attributes(),
    { z => 3 },
...
note 'Class::XSAccessor: ', fields::LOP::XS ? 1 : 0;
