...
A pure Perl based toolkit was used to ensure that the applications remain 
efficient and portable. The L<fields>/L<base> pragmas (by default) and 
L<UNIVERSAL::Object> and L<Moo> and L<Moose> are currently supported.

The port allows developers to extend the functionality of Turbo Vision through 
Perl modules and libraries.

Note that Moo/se support has only been tested a very limited amount.
...
Detailed documentation and examples can be found in the GitHub repository of the 
port.
...
Copyright (c) 2020-2025 the AUTHOR and CONTRIBUTORS.
