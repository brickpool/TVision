...
sub new {    # $obj (%args)
  state $sig = signature(
    method => 1,
    named => [
      data      => Maybe[Str],               { optional => 1 },
      size      => Maybe[PositiveOrZeroInt], { optional => 1 },
...
sub from {    # $obj ($tp|$d, $len)
  state $sig = signature(
    ...
    pos    => [
      Defined,    # Str|Object
      PositiveOrZeroInt, { optional => 1 }
    ],
  );
  my ( $class, @args ) = $sig->( @_ );
  return is_Object $args[0]
    ? $class->new( copy_from => $args[0] )
    : $class->new( data => $args[0], size => $args[1] );
  return;
}
