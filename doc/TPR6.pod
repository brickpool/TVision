...
=head1 CHAPTER 6 - The Turbo Vision Reference

Chapter 6 of the Turbo Pascal Reference
...
TResourceFile, TStringList and TStrListMaker data structure objects. 

The line of descent shown in the illustration below identifies how functionality 
is inherited between the objects. For example, TDialog is descended from TWindow
...
For the object descriptions, the information is organized into sections:
...
other facilities of the object. 

Many Turbo Vision objects have a large variety of fields and methods available 
for use. 

However, many of these are needed only for the implementation of Turbo Vision 
itself, and in most practical applications you will only need to reference a 
small selection of the fields and methods.
...
TApplication->new, via TProgram->BUILD, sets L</$application> to $self, hence,
L</$application> points to the application object. TProgram->DESTROY resets
L</$application> to C<undef>.
...
Holds the current application color palette selection (either C<apColor>,
C<apBlackWhite> or C<apMonoChrome>). 

B<See>: TView->setScreenMode, C<apXXXX> constants
...
These values are kept in L</$appPalette>, indicating which application color 
palette is in use.
...
The C<bfXXXX> values are passed as a bit mask to TButton->new and specify button
...
If a mouse is installed, L</$buttonCount> holds the number of buttons on the 
mouse. If zero, then no mouse is installed. 

Check the value of L</$buttonCount> when your program needs to know if a mouse 
is installed.
...
If a CGA adaptor is detected, Turbo Vision sets $checkSnow to C<TRUE>. Older CGA
...
$checkSnow to C<FALSE>, resulting in faster output to the screen. Do not set or 
check $checkSnow until after calling TApplication->new.
...
color palettes. Except for C<cpColor>, C<cpBlackWhite> and C<cpMonoChrome>, all 
of the color palette constants contain indices into their owner's palette. For 
C<cpColor>, C<cpBlackWhite> and C<cpMonoChrome>, each entry contains an actual 
BIOS video color attribute byte.
...
palette. This color entry is an index into its owner's palette, C<cpDialog>.
Looking at the 10th entry of C<cpDialog>, we see that this maps to entry 41 in 
the application-level palette. In the case of C<cpColor> (color display in use),
this corresponds to black lettering on a green background.
...
The C<cmXXXX> constants are the predefined commands within Turbo Vision and are
passed to handleEvent methods in the $event->{command} field. 

Constant values of 0 to 99, and 256 to 999 are reserved for use by Turbo Vision.
Your applications can define command constants in the range 100 to 255, and 1000 
to 65535.
...
short-cut characters surrounded by tilde C<~> characters, minus the number of
...
You can clear it any time by resetting to C<FALSE>.
...
To assist in providing Wordstar compatibility, ctrlToArrow can be used to 
convert $event->{keyCode} values to their corresponding C<kbXXXX> values. 

The following chart shows the mapping from control keys to C<kbXXXX> values.
...
contain the top scan line and the low 4 bits contain the bottom scan line. 

For example, on a CGA screen a character is made up of 8 scan lines, with the
topmost scan line being 0 and the bottommost scan line being 7 (the number of
scan lines varies according to the type of the video adaptor card in use). 

The shape of the character cursor is determined with BIOS interrupt <0x10>,
function <0x01>. For more information, consult the DOS Programmer's Reference, 
2nd  Edition, Que Books, page 424 "BIOS INT 10H Function 01h Set Cursor Type". 
...
resume disables the mouse interrupt handler and hides the mouse.

=head2 THistory->doneHistory
...
vectors 0x09, 0x1b, 0x21, 0x23 and 0x24 and the Ctrl-Break checking of DOS, to 
...
L</$doubleDelay> holds the time interval (in 1/18.2 of a second intervals) 
defining how quickly two mouse clicks must occur in order to be treated as a 
...
considered a double click event (with TEvent->{double} set to C<TRUE>).
...
this value. C<0xCF> is the video attribute for flashing white characters on a 
red background - if you see flashing white on red, you need to check or override
the getPalette method. 

An example of how this works can be seen in the C<TVSHELL> program. If the 
TDirList->getPalette method is removed from TDirList, the listbox text will 
appear as flashing white on red. 

This occurs because the palette indices for a TListViewer, 26, 27, 28 and 29 are
out of range for the window palette into which the list viewer is inserted.

The solution is to override getPalette to return values within an acceptable 
range.
...
The C<evXXXX> constants are used to designate event types and event masks,
...
C<positionalEvents> and L</focusedEvents>.
...
L</focusedEvents> contains a bit mask for identifying events that are focused. 
Any event that is neither a focused event nor a positional event, is classed as 
a broadcast event. 

B<See>: C<positionalEvents> constant, and "Positional Events" in Chapter 13, 
...
=head3 Declaration
    ...
    sub TVMemMgr::freeDiscardable;    # void ($p)
...
Frees up the cache buffer pointed to by $p. See L</allocateDiscardable> for 
details.
...
prints the single letter C<A>. Procedure getAltCode maps characters back to 
Alt-Ch combinations.
...
Maps a single character C<A> to C<Z> to the Keycode equivalent value for 
pressing Alt-Ch. For example,

    getAltCode('A')

returns C<0x1E00>, which is the value of C<kbAltA>. getAltCode is the inverse 
...
=head3 Declaration
    ...
    sub TVMemMgr::allocateDiscardable;    # void ($p; $size)
...
Used to allocate a cache buffer where the cache buffer is a heap allocation that
may be moved or disposed of at any time by the memory manager. As a result, the 
pointer variable passed to L</allocateDiscardable> may be changed by the memory 
manager at any time during program execution to point to a different memory 
area, or even be reset to C<undef>.

Turbo Vision uses cache buffers to speed up output the screen (also see 
TView->options field and the C<ofXXXX> constants) by having views store their 
display image in memory so that, if possible, the screen can be updated quickly 
merely by copying the memory image back to the screen (this is faster than 
regenerating the text and other data in the Draw method).

However, when Turbo Vision begins to run low on memory, it starts throwing away 
the cache buffers, as needed until enough memory is freed up.

In addition to Turbo Vision's use of cache buffers, your application can use 
cache buffers for storage that can be deleted by the memory manager in case 
memory is running low. An example use might be to cache the disk directory read 
into the C<TVSHELL8> sample program of Chapter 12 in the Borland Pascal 
Developer's Guide.
...
undef. Since the C<TVSHELL> program can always recreate the directory structure 
by reading the disk directory again, its okay to reallocate this memory to some
other function.

Cache buffers are useful for speeding up access to items that can always be 
recreated.

B<See>: L</freeDiscardable>.
...
Uses the BIOS function INT 0x16, Function 0x01 "Read Keyboard Status" to
determine if a key has been pressed on the keyboard. If so, $event->{what} is 
set to C<evKeyDown> and $event->{keyCode} is set to the scan code of the key. If
no keys have been pressed, $event->{what} is set to C<evNothing>. This is an 
internal subroutine called by TProgram->getEvent. 

B<See>: C<evXXXX> constants, also DOS Programmer's Reference, 2nd Edition, Que 
Books, page 496,
...
Similar to L</getKeyEvent>, but for mouse events. This internal subroutine 
checks Turbo Vision's internal mouse event queue, and if a mouse event has 
occurred, sets $event->{what} to the appropriate C<evMouseXXXX> constant; 
$event->{buttons} to C<mbLeftButton> or C<mbRightButton>; $event->{double} to 
C<TRUE> or C<FALSE>; and $event->{where} to the mouse position in TApplication 
coordinates. If no mouse events have occurred, $event->{what} is set to 
C<evNothing>.
...
The C<gfXXXX> constants set the TView->growMode field, which controls how a view
